...
class AuthService:
    ...
    def _dispatch_otp(self, identifier: str, identifier_type: str, raw_otp: str) -> str | None:
        ...
        raise HTTPException(
            ...
            detail=AuthError.SMS_NOT_CONFIGURED,
        )
    ...
    def verify_admin_otp(
        ...
        if not user or not user.is_active:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=AuthError.ADMIN_INACTIVE,
            )
    ...
    async def google_login_admin(
        ...
        if not google_data:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=AuthError.INVALID_GOOGLE_TOKEN,
            )
    ...
    def refresh_session(
        ...
        if not raw_refresh_token:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=AuthError.REFRESH_MISSING,
            )
        ...
        if not session:
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=AuthError.REFRESH_INVALID,
            )
        ...
        if now >= expires_at:
            ...
            raise HTTPException(
                status_code=status.HTTP_401_UNAUTHORIZED,
                detail=AuthError.REFRESH_MAX_AGE,
            )
