...
class FinancialTransaction(BaseEntity):
    ...
    __table_args__ = (
        CheckConstraint(
            "amount > 0",
            name="ck_financial_transactions_amount_positive"
        ),
        ...
        Index(
            "ix_financial_transactions_external_reference",
            "external_reference"
        ),
    )
    ...
    category: Mapped[FinancialTransactionCategory | None] = mapped_column(
        Enum(FinancialTransactionCategory, name="financial_transaction_category", create_type=False), 
        nullable=True, 
        index=True
    )
